...
- `POST /admin/generate-post`도 즉시 `{"status":"started"}`를 돌려주고 백그라운드로 돈다. 실패하면 2분 간격으로 5회까지 재시도한다(최대 10분). 휴장일이면 재시도 없이 종료. 18:30 재호출이 겹치면 락으로 건너뛴다
  - ⚠️ **KRX 전종목 API가 간헐적으로 실패한다.** 2026-08-18 시황이 이것 때문에 통째로 누락됐다. 전종목 API는 당일 데이터만 주므로 **지나간 날은 복구할 수 없다**. 실패가 반복되면 크론 시각을 19:00~20:00으로 늦추는 것을 검토할 것 (17:30~18:30이 KRX 혼잡 시간대로 보인다)
...
- **Render에서 KRX 전종목 API(`data.krx.co.kr`)가 차단된다** (2026-08-18 확인). 로컬(한국)은 정상. 그래서 `backend/snapshot/stocks.csv`로 폴백한다. 개별 종목 가격 히스토리는 정상 수신되므로 동조 분석은 영향 없다. 시세·시총은 스냅샷 시점 값으로 고정되니, 신규 상장이 쌓이면 로컬에서 CSV를 다시 만들어 커밋할 것
