...
- `POST /admin/generate-post`도 즉시 `{"status":"started"}`를 돌려주고 백그라운드로 돈다. 실패하면 2분 간격으로 5회까지 재시도한다(최대 10분). 휴장일이면 재시도 없이 종료. 18:30 재호출이 겹치면 락으로 건너뛴다
  - ⚠️ **KRX 전종목 API가 간헐적으로 실패한다.** 2026-08-18 시황이 이것 때문에 통째로 누락됐다. 전종목 API는 당일 데이터만 주므로 **지나간 날은 복구할 수 없다**. **2026-08-19에 크론을 19:00/20:00으로 늦췄다.** 실패가 몰린 시각이 전부 16:27~18:30이었고, 같은 날 오전에 수동 호출했을 때는 20초 만에 성공했다. 장 마감 직후가 KRX 혼잡 시간대로 보인다
